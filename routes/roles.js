...
// CREATE - Tạo role mới (yêu cầu đăng nhập)
...
// UPDATE - Cập nhật role theo ID (yêu cầu đăng nhập)
...
// DELETE - Xóa role theo ID (yêu cầu đăng nhập)
...
router.get('/:id/users', async function (req, res, next) {
    try {
        ...
        let users = await userModel.find({ role: roleId, isDeleted: false }).populate('role').select('-password');
        res.send(users);
    } catch (error) {
        res.status(500).send({ message: error.message });
    }
